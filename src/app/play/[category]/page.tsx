...
function ChatBox({ room, lang, currentPrompt, setCurrentPrompt, setSeenPromptIds, remoteItems, prompts, showToast }: ChatBoxProps) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-4 flex justify-center pointer-events-none">
      <div className="pointer-events-auto w-[92%] max-w-2xl rounded-xl bg-black/80 backdrop-blur-md shadow-[0_10px_30px_rgba(168,85,247,.25)] p-3 flex flex-col max-h-[160px] sm:max-h-[300px] md:max-h-[320px]">
...
function ChatBoxIntimacy({ room, lang, currentPrompt, setCurrentPrompt, setSeenPromptIds, remoteItems, prompts, showToast }: ChatBoxProps) {
  const [loaded, setLoaded] = useState(false);
  const [items, setItems] = useState<Array<{ id: string; user_id: string; nickname?: string; text: string; created_at: string }>>([]);
  const [myId, setMyId] = useState<string>("");
  ...
  useEffect(() => {
    if (!room) return; const now = Date.now(); if (isTyping && now - lastTypingSentAtRef.current > 1200) { lastTypingSentAtRef.current = now; try { channelRef.current?.send({ type: 'broadcast', event: 'typing', payload: { user_id: myId, prompt_id: promptId || null } }); } catch {} }
  }, [isTyping, room, myId, promptId]);
  ...
  return (
    <div className="fixed inset-x-0 bottom-4 flex justify-center pointer-events-none">
      <div className="pointer-events-auto w-[92%] max-w-2xl rounded-xl bg-black/80 backdrop-blur-md shadow-[0_10px_30px_rgba(225,29,72,.25)] p-3 flex flex-col max-h-[160px] sm:max-h-[300px] md:max-h-[320px]">
        {needNick && typeof window !== 'undefined' && createPortal((
          <div className="fixed inset-0 z-[10000] flex items-center justify-center">
            <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" />
            <div className="relative z-10 w-[92%] max-w-sm rounded-xl bg-black/90 text-white p-5 border border-rose-600/60">
              <h3 className="text-lg font-semibold mb-2">{lang==='en'?'Set your nickname':'请输入昵称'}</h3>
              <p className="text-sm opacity-80 mb-4">{lang==='en'?'This will be shown to your partner in this room only.':'只用于当前房间展示，不会被保存。'}</p>
              <div className="flex items-center gap-2">
                <input ref={nickInputRef} autoFocus value={nick} onChange={(e)=>setNick(e.target.value)} placeholder={lang==='en'?'Nickname':'昵称'} className="flex-1 px-3 py-2 rounded border border-rose-600/60 bg-black/60 text-base text-white placeholder:text-white/40" />
                <button onClick={()=>{ if(nick.trim()){ try { nickInputRef.current?.blur(); } catch {} setNeedNick(false); try { sessionStorage.setItem(`chat_nick_set_${room}`,'1'); sessionStorage.setItem(`chat_nick_${room}`, nick.trim()); } catch {} } }} className="px-4 py-2 rounded-full bg-rose-600 text-white text-sm hover:brightness-110 disabled:opacity-50" disabled={!nick.trim()}>{lang==='en'?'Confirm':'确定'}</button>
              </div>
              <div className="mt-3 flex items-center justify-between">
                <button onClick={copyLinkInModal} className="px-3 py-1.5 rounded-full border border-rose-600/60 text-xs hover:bg-white/10">{lang==='en'?'Copy Link':'复制链接'}</button>
                {copiedInModal === true && (<span className="text-xs text-rose-200/90">{lang==='en'?'Copied. Please share the invite.':'复制成功，请分享邀请。'}</span>)}
                {copiedInModal === false && (<span className="text-xs text-red-300/90">{lang==='en'?'Copy failed':'复制失败'}</span>)}
              </div>
            </div>
          </div>
        ), document.body)}
        <div className="text-xs text-rose-200/80 mb-2 flex items-center justify-between">
          <span>{lang === 'en' ? 'Chat' : '聊天'} {room ? `#${room}` : ''}</span>
          {!room && <span className="opacity-70">{lang === 'en' ? 'Create a chat to start' : '创建房间后开始聊天'}</span>}
        </div>
        <div ref={listRef} className="flex-1 min-h-0 overflow-auto space-y-2 pr-1 no-scrollbar">
...
function ChatBoxParty({ room, lang, currentPrompt, setCurrentPrompt, setSeenPromptIds, remoteItems, prompts, showToast }: ChatBoxProps) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-4 flex justify-center pointer-events-none">
      <div className="pointer-events-auto w-[92%] max-w-2xl rounded-xl bg-black/80 backdrop-blur-md shadow-[0_10px_30px_rgba(234,179,8,.25)] p-3 flex flex-col max-h-[160px] sm:max-h-[300px] md:max-h-[320px]">
        {needNick && typeof window !== 'undefined' && createPortal((
          <div className="fixed inset-0 z-[10000] flex items-center justify-center">
            <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" />
      ...
      <div className="w-full max-w-2xl text-center mt-10">
        ...
        {currentPrompt ? (
          <div className={`rounded-lg border ${theme.cardBorder} p-6 text-left bg-black/20 backdrop-blur-[2px] ${theme.cardShadow} card-breathe mt-10`}>
            ...
            <div className="mt-6 text-xs opacity-70">{isPro ? (lang === "en" ? "All questions unlocked" : "已解锁全部问题") : (lang === "en" ? "Currently at trial mode. Unlock for full access." : "当前为体验版，完整版请解锁。")}</div>
          </div>
        ) : (
