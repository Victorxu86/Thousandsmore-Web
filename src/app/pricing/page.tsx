...
export default function PricingPage() {
  ...
  return (
    <div className="min-h-screen p-6 sm:p-10 max-w-3xl mx-auto">
      ...
      <h1 className="text-2xl font-semibold mb-4 text-purple-200">{lang === "en" ? "Pricing" : "定价"}</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        {[
          ...
          <div key={c.id} className="rounded-lg border border-purple-500/40 p-6 bg-black/20 backdrop-blur-[2px] shadow-[0_8px_30px_rgba(88,28,135,0.25)]">
            <div className="text-lg font-medium mb-2">{lang === "en" ? `Unlock ${c.en}` : `解锁 ${c.zh}`}</div>
            <div className="opacity-80 mb-3">{lang === "en" ? "Unlimited access to the full category" : "该分类无限次数与完整题库"}</div>
            <div className="flex items-baseline gap-3 mb-4">
              ...
              {c.old !== c.now && (
                <span className="text-[10px] px-2 py-0.5 rounded-full border border-purple-500/60 opacity-80">{lang === "en" ? "Limited" : "限时"}</span>
              )}
            </div>
            <button
        ...
        <div className="rounded-lg border border-purple-500/40 p-6 bg-black/20 backdrop-blur-[2px] shadow-[0_8px_30px_rgba(88,28,135,0.25)]">
          <div className="text-lg font-medium mb-2">{lang === "en" ? "Unlock All" : "全部解锁"}</div>
          <div className="opacity-80 mb-3">{lang === "en" ? "One purchase, unlock all categories" : "一次购买，解锁全部分类与次数限制"}</div>
          <div className="flex items-baseline gap-3 mb-4">
